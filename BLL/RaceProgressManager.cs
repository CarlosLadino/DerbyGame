...
namespace BLL
{
    public class RaceProgressManager
    {
        ...
        public RaceProgress Find(int raceId, int placeId, int timeMarker)
        {
            return this._context.RaceProgress.Where(e => e.RaceId == raceId && e.PlaceId == placeId && e.TimeMarker == timeMarker).FirstOrDefault();
        }
        ...
        {
            bool isOk = false;
            var firstPlace = this.Find(vwRecord.RaceId, 1, vwRecord.TimeMarker);
            if (firstPlace != null)
            {
                this._context.Remove(firstPlace);
            }
            var secondPlace = this.Find(vwRecord.RaceId, 2, vwRecord.TimeMarker);
            if (secondPlace != null)
            {
                this._context.Remove(secondPlace);
            }
            var thirdPlace = this.Find(vwRecord.RaceId, 3, vwRecord.TimeMarker);
            if (thirdPlace != null)
            {
        ...
        private bool IsUnique(RaceProgress record, ICollection<ValidationResult> errorMessages)
        {
            ...
            recordsFound = this._context.RaceProgress.Any(e => e.RaceId == record.RaceId && e.TimeMarker == record.TimeMarker && e.PlaceId == record.PlaceId && e.HorseNumber == record.HorseNumber && e.Id != record.Id);

            if ( recordsFound)
            {
                errorMessages.Add(new ValidationResult("A record with the same race, place, time marker and horse already exists."));
            }
        ...
        {
            var record = this.Find(raceId, placeId, timeMarker);
            if (record != null)
            {
